...
import { 
  ...
  RefreshCw, Wifi, WifiOff, FileText, Filter, AlertCircle, ArrowLeft 
} from 'lucide-react';
...
export default function ForestDashboard() {
  ...
  const handleLoadSampleSocial = () => {
    const samples = 
`Tree fallen across reserve track in Jim Corbett @29.53,78.77\nUnusual tree cutting reported near reserve area forest border\nForest fire spreading rapidly near Western Ghats hill ridge @12.29,76.63\nPoachers suspected in Kaziranga restricted zone patrol spot @26.57,93.17\nSmuggling of sandalwood tree timber trunks seen at night\nStrong wind storm blow down pine forest trees`;
  ...
  const handleRunNlp = () => {
    ...
    lines.forEach((line, i) => {
      const type = classifyText(line, 'forest');
      counts[type] = (counts[type] || 0) + 1;

      // Extract coords
      const m = line.match(/@(-?\d+\.\d+),\s*(-?\d+\.\d+)/);
      const lat = m ? parseFloat(m[1]) : 10 + Math.random() * 20; // Simulated India lat bounds
      const lng = m ? parseFloat(m[2]) : 70 + Math.random() * 20; // Simulated India lng bounds
      ...
      <div className="flex-grow flex flex-col lg:flex-row overflow-hidden relative z-10 w-full h-[calc(100vh-76px)]">
        ...
        <section className="w-full lg:w-4/12 p-4 shrink-0 flex flex-col gap-4 overflow-y-auto lg:h-full border-l border-slate-950 bg-slate-950/20">
          ...
          <div className="glass-panel p-4 rounded-xl space-y-4 shrink-0">
            <h3 className="text-sm font-bold text-emerald-400 uppercase tracking-widest border-b border-slate-900 pb-2 mb-1">
              {t('socialMonitor')}
            </h3>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-grow py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-450 hover:to-teal-500 text-white font-bold text-xs flex items-center justify-center gap-1.5 cursor-pointer glow-btn"
              >
                <Wand2 size={13} />
                {t('analyze')}
              </button>
                ...
                <div className="space-y-1.5 max-h-[100px] overflow-y-auto pr-1">
                  {nlpResults.map((r, idx) => (
                    <div key={idx} className="flex justify-between items-center text-[11px] text-slate-300 py-1 border-b border-slate-900/40 last:border-0">
                      <span className="font-semibold text-emerald-400 uppercase">{r.type}</span>
